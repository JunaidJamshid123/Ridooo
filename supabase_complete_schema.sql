-- ============================================================================
-- RIDOOO - Complete Database Schema for Supabase
-- ============================================================================
-- Run this script in Supabase SQL Editor
-- Make sure to run sections in order due to foreign key dependencies
-- ============================================================================
...
-- Function to update driver rating after new rating
...
BEGIN
    ...
    SELECT AVG(r.rating)::DECIMAL(3,2) INTO avg_rating
    FROM ratings r
    JOIN rides ri ON r.ride_id = ri.id
    WHERE ri.driver_id = (SELECT driver_id FROM rides WHERE id = NEW.ride_id)
    AND r.reviewee_id = (SELECT driver_id FROM rides WHERE id = NEW.ride_id);
...
-- ============================================================================
-- SECTION 3: CORE TABLES (Already created - included for reference)
-- ============================================================================

-- Note: If these tables already exist, comment out or skip this section
...
-- ============================================================================
-- SECTION 4: VEHICLE & FARE CONFIGURATION
-- ============================================================================

-- Vehicle Types (bike, car, premium, suv, etc.)
...
-- Fare Configuration per vehicle type and city
...
-- ============================================================================
-- SECTION 5: PROMO CODES & DISCOUNTS
-- ============================================================================
...
-- ============================================================================
-- SECTION 6: RIDES TABLE (Updated with more fields)
-- ============================================================================

-- Drop existing rides table if you want to recreate with new fields
-- DROP TABLE IF EXISTS rides CASCADE;

-- Rides table (if not exists or recreating)
...
-- Add foreign key for user_promo_usage if rides table was recreated
ALTER TABLE user_promo_usage 
    DROP CONSTRAINT IF EXISTS user_promo_usage_ride_id_fkey;
ALTER TABLE user_promo_usage 
    ADD CONSTRAINT user_promo_usage_ride_id_fkey 
    FOREIGN KEY (ride_id) REFERENCES rides(id);

-- ============================================================================
-- SECTION 7: RIDE REQUESTS (Multi-driver broadcast)
-- ============================================================================

-- Ride Requests (sent to multiple nearby drivers)
...
-- ============================================================================
-- SECTION 8: DRIVER LOCATIONS (Real-time tracking)
-- ============================================================================
...
CREATE TABLE wallet_transactions (
    ...
    reference_id UUID,  -- Can reference ride_id or other entities
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Ratings (one per ride, but can have user rating driver AND driver rating user)
CREATE TABLE ratings (
    ...
    comment TEXT,
    tags TEXT[],  -- ['friendly', 'clean_car', 'good_driving', etc.]
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(ride_id, reviewer_id)  -- One rating per ride per reviewer
);

-- ============================================================================
-- SECTION 11: SAVED PLACES
-- ============================================================================
...
CREATE TABLE saved_places (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES users(id) NOT NULL,
    label TEXT NOT NULL,  -- 'home', 'work', 'gym', custom label
    name TEXT,
...
CREATE TABLE notifications (
    ...
    data JSONB,  -- Additional data like ride_id, etc.
    action_url TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE support_tickets (
    ...
    assigned_to TEXT,  -- Support agent email/id
    resolution TEXT,
...
CREATE TABLE support_messages (
    ...
    sender_id TEXT,  -- user_id or support agent id
...
-- ============================================================================
-- SECTION 14: APP SETTINGS
-- ============================================================================

-- App Settings (Key-value store for configurations)
...
-- Driver Locations (for geospatial queries)
...
-- View for ride details with user and driver info
...
SELECT 
    ...
    SUM(CASE WHEN r.completed_at >= NOW() - INTERVAL '1 day' THEN p.amount ELSE 0 END) as today_earnings,
    SUM(CASE WHEN r.completed_at >= NOW() - INTERVAL '7 days' THEN p.amount ELSE 0 END) as week_earnings,
    SUM(CASE WHEN r.completed_at >= NOW() - INTERVAL '30 days' THEN p.amount ELSE 0 END) as month_earnings,
...
-- ============================================================================
-- SECTION 19: STORAGE BUCKETS (Run in Storage section or via API)
-- ============================================================================

-- Note: Create these buckets in Supabase Dashboard -> Storage
-- 1. profile-images (for user profile pictures)
-- 2. documents (for driver documents like license)
-- 3. support-attachments (for support ticket attachments)
...
-- ============================================================================
-- COMPLETE! Your Ridooo database is ready.
-- ============================================================================
...
2.  drivers            - Extended driver info
3.  driver_locations   - Real-time GPS tracking
4.  vehicle_types      - Available vehicle categories
5.  fare_configs       - Pricing configuration
6.  promo_codes        - Discount codes
7.  user_promo_usage   - Track promo usage
8.  rides              - Core ride bookings
9.  ride_requests      - Multi-driver broadcast
10. wallets            - User wallet balance
11. wallet_transactions- Wallet history
12. payments           - Ride payments
13. ratings            - User/Driver ratings
14. saved_places       - Saved addresses
15. notifications      - Push notifications
16. support_tickets    - Customer support
17. support_messages   - Support conversations
18. cancellation_reasons - Lookup table
19. app_settings       - System configuration

NEXT STEPS:
===========
1. Run this script in Supabase SQL Editor
2. Create storage buckets in Supabase Dashboard
3. Update your Flutter app's supabase_config.dart with new table names
4. Implement the data layer for each new entity
*/
